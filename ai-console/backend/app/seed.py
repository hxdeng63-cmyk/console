...
async def seed_algorithms(session: AsyncSession) -> list[int]:
    ...
    algo_data = [
        ("traffic", "detection", "traffic", "交通事件检测算法", [
            ("jam", "交通阻塞"),
            ("anomaly", "异常停车"),
            ("flow", "流量统计"),
            ("reverse", "逆向行驶"),
...
async def seed_warning_events(session: AsyncSession) -> list[int]:
    ...
    # Seeded events have no media by default; media is generated by traffic modules.
    # Using None avoids 404s from files that may not exist on disk.
    local_images = [None]
    local_videos = [None]
    ...
    for device in devices:
        ...
        for _ in range(event_count):
            ...
            # Image/video URLs: no media for seeded events
            idx = len(events)
